...
class SSP {
	 ...
	 */
	static function filter ( $request, $columns, &$bindings, $custom_where, $group_cond=NULL)
	{
	...
	static function simple ( $request, $table, $primaryKey, $columns, $custom_where='', $secondary_database='', $group_cond='')
	{
		...
		$where = SSP::filter( $request, $columns, $bindings, $custom_where, $group_cond);
		...
		**/
		...
		else
		{
			...
			if(is_array($bindings)){
				...
				//$bindings = array('234234234','32482347293429');
				$webDB->query($sql,$bindings);
				//echo $webDB->_compile_select();die();
				//print_r($webDB->error());
				//echo $webDB->last_query();
